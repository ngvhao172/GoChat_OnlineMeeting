...
class RoomController {
  ...
  joinRoom = async (req, res, next) => {
    ...
    try {
      ...
      ws.onopen = () => {
        ws.send(
          JSON.stringify({
            action: "check-room-existed",
            roomId: roomId.toString().trim(),
            userEmail: res.locals.user.userEmail,
          })
        );
      };
      ws.onmessage = async (message) => {
        ...
        switch (data.action) {
          case "checked-result":
            if (data.exists) {
              ...
              if (
                (isRoomNewCreated && isRoomNewCreated[0] == true) ||
                (isPrejoined && isPrejoined[0] == true)
              ) {
                try {
                  ...
                  if (data.isApproved) {
                    ...
                    await roomService.updateRoom(roomData.data.id, {
                      attendees: attendees,
                    });
  ...
  async getRoomsByDay(req, res, next) {
    try {
      ...
      // }
      const rooms = await roomService.getRoomsByUserEmailAndWeekDay(
        day,
  ...
  async updateRoom(req, res, next) {
    try {
      ...
      // const roomData = await roomService.getRoomByRoomId(roomId);
      // if(roomData.status){
      let updateRoom = await roomService.updateRoom(id, {
        roomName: roomName,
        startAt: startAt,
        endAt: endAt,
    ...
    try {
      ...
      if (roomData.status) {
        if (roomData.data.attendees[0] == res.locals.user.userEmail) {
          ...
          let updateRoom = await roomService.updateRoom(roomData.data.id, {
            attendees: attendees,
          });
          if (updateRoom.status) {
            return res.json(updateRoom);
          ...
          case "created":
            if (data.status) {
              ...
              try {
                ...
                return res
                  .status(400)
                  .json("Error when creating later meeting: " + error);
              }
            } else {
              // console.log(data.message);
              return res
